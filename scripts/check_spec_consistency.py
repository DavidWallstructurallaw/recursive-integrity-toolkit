...
def phase4_current_main(step: int = 10) -> int:
    ...
    if type(step) is not int or step != 10:
        raise ValueError("Only authorized Phase 4 Step 10 specification checking is available")
    control = runpy.run_path(str(ROOT / "scripts/release_check.py"),
                            run_name="phase4_current_specification_control")
    control["audit_phase4_current"](step=step)
    result = main()
    print("Phase 4 Step 10: performance evidence and preserved specification structure: PASS")
    return result
